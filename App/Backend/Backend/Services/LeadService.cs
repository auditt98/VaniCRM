...
namespace Backend.Services
{
    public class LeadService
    {
        ...
        public LeadListApiModel GetLeadList(string query = "", int pageSize = 0, int currentPage = 1, List<string> sort = null)
        {
            ...
            apiModel.leads = dbLeads.leads.Select(c => new LeadListApiModel.LeadInfo() { id = c.ID, name = c.Name, companyName = c.CompanyName, email = c.Email, phone = c.Phone, leadSource = c.LEAD_SOURCE != null ? c.LEAD_SOURCE.Name : "", leadOwner = c.Owner?.Username, priority = c.PRIORITY != null ? c.PRIORITY.Name : "", avatar = c.Avatar != null ? $"{StaticStrings.ServerHost}avatar?fileName={c.Avatar}" : $"{StaticStrings.ServerHost}avatar?fileName=default.png" }).ToList();
