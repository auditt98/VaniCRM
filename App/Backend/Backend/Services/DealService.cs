    ...
    public class DealService
    {
        ...
        public DealDetailApiModel GetOne(int id)
        {
            ...
            if (dbDeal != null)
            {
                ...
                apiModel.priorities = _priorityRepository.GetAllPriorities().Select(c => new PrioritySelectionApiModel() { id = c.ID, name = c.Name, selected = dbDeal.PRIORITY != null ? dbDeal.PRIORITY.ID == c.ID : false }).ToList();
                //notes
